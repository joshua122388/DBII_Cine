...
public class LoginAdmin extends javax.swing.JFrame {
    ...
    private void initComponents() {
        ...
        getContentPane().add(jLabel1, new org.netbeans.lib.awtextra.AbsoluteConstraints(40, 160, 110, -1));
        getContentPane().add(txtuserAdmin, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 90, 180, -1));
        ...
        getContentPane().add(txtadminPassword, new org.netbeans.lib.awtextra.AbsoluteConstraints(150, 160, 180, -1));
...
String usuario = txtuserAdmin.getText().trim();
String contrasena = new String(txtadminPassword.getPassword()).trim();

        try (Connection conn = ConexionSQL.conectar()) {
            // Encriptar la contraseña ingresada
        Encriptador encriptador = new LogicaNegocio.EncriptadorSHA256();
        String hashIngresado = encriptador.encriptarSHA256(contrasena);

    CallableStatement stmt = conn.prepareCall("{call sp_LoginAdministrador(?, ?, ?)}");
    stmt.setString(1, usuario);
    stmt.setString(2, hashIngresado);
    stmt.registerOutParameter(3, java.sql.Types.BIT);

    stmt.execute();

    boolean accesoPermitido = stmt.getBoolean(3);

    if (accesoPermitido) {
        JOptionPane.showMessageDialog(this, "Inicio de sesión exitoso. Bienvenido administrator.");
