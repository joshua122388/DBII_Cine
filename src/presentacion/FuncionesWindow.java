...
private void cargarFunciones() {
    ...
    private void initComponents() {
        ...
        brnFiltro.addActionListener(new java.awt.event.ActionListener() {
            public void actionPerformed(java.awt.event.ActionEvent evt) {
                brnFiltroActionPerformed(evt);
            }
        });
    ...
    private void brnFiltroActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_brnFiltroActionPerformed
    try {
        // Obtener el ID de la película desde el JComboBox
        String seleccion = cmbPelicula.getSelectedItem().toString();
        int idPelicula = Integer.parseInt(seleccion.split(" - ")[0].trim());

        // Conectar a la base de datos
        Connection conn = ConexionSQL.conectar();

        // Consulta SQL para funciones de una película específica
        String query = "SELECT fu.Fecha, pel.Titulo, fu.Hora_Inicio, fu.Duracion, fu.Numero_Sala " +
                       "FROM funcion fu JOIN pelicula pel ON fu.ID_Pelicula = pel.ID_Pelicula " +
                       "WHERE fu.ID_Pelicula = ?";
        PreparedStatement stmt = conn.prepareStatement(query);
        ...
        tblFunciones.setModel(model); // Aplicar modelo actualizado
    ...
    }//GEN-LAST:event_brnFiltroActionPerformed
